...
| `export_apk(project)` | Gebaute APK nach [`/exchange`](austausch-ablage.md) kopieren (nicht mit dem Dateisystem-Werkzeug `move_file` möglich — siehe unten) |
...
- **Eine gebaute APK NICHT mit dem Dateisystem-Werkzeug `move_file` nach `/exchange` verschieben** — `/workspace` und `/exchange` sind zwei verschiedene Docker-Volumes, das scheitert dort mit "Invalid cross-device link" (ein `rename()`-Systemaufruf funktioniert nur innerhalb desselben Dateisystems). Dafür gibt es `export_apk(project)`, das direkt in diesem Dienst kopiert.
